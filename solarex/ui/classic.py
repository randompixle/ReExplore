...
class ClassicWindow(QtWidgets.QMainWindow):
    def __init__(self, core, start_url: str | None = None):
        super().__init__()
        self.core = core
        self.setWindowTitle("SolarEx" + (" (Incognito)" if core.profile.incognito else ""))
        ...
        bar.addWidget(self.go, 0)
        bar.addWidget(self.newtab, 0)
    ...
    def open_tab(self, url: str):
        ...
        if hasattr(view, "loadFinished"):
            view.loadFinished.connect(lambda ok=None, v=view: self.addr.setText(v.url().toString() if hasattr(v, "url") else self.addr.text()))
    ...
    def load_from_entry(self):
        ...
        elif w and hasattr(w, "setSource"):
            w.setSource(QtCore.QUrl(url))

    # used by RendererSettings to swap active backend
    def swap_current_view(self, backend_name: str):
        cur = self.tabs.currentWidget()
        url = None
        try:
            if hasattr(cur, "url"): url = cur.url().toString()
        except Exception:
            pass
        if not url:
            url = self.addr.text() or "https://www.google.com/"
        new_view = self.core.render.new_view()
        idx = self.tabs.currentIndex()
        self.tabs.removeTab(idx)
        self.tabs.insertTab(idx, new_view, "New Tab")
        self.tabs.setCurrentIndex(idx)
        if hasattr(new_view, "load"):
            new_view.load(QtCore.QUrl(url))

def init(core):
    core.ui = ClassicWindow
